...
export default function PendingListings() {
    ...
    const fetchListings = async () => {
      try {
        const res = await fetch(
          "http://localhost:3000/api/get-pending-requests",
          {
            method: "GET",
            headers: {
        ...
        setListings(data.pending || []); // use 'pending' based on your API response
      } catch (err) {
  ...
  return (
    <main className="max-w-6xl mx-auto p-4">
      <h1 className="text-3xl font-bold text-center mb-8">Pending Listings</h1>
